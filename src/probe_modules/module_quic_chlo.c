...
#define PUBLIC_FLAG_8BYTE_CONN_ID 0x0C
	...
	uint32_t
	    quic_version; // should be MakeQuicTag('Q', '0', '2', '5') but server may provide list
	uint8_t
...
void chlo_quic_set_num_ports(int x) { num_ports = x; }
...
int chlo_quic_global_initialize(struct state_conf *conf)
{
	...
	module_quic_chlo.pcap_snaplen =
	    sizeof(struct ether_header) + sizeof(struct ip) +
	    sizeof(struct udphdr) + QUIC_HDR_LEN_HASH + STREAM_FRAME_LEN +
	    INCHOATE_CHLO_LEN + CLIENTHELLO_MIN_SIZE;

	connection_id = MakeQuicTag('S', 'C', 'A', 'N') |
			((uint64_t)MakeQuicTag('N', 'I', 'N', 'G')) << 32;
...
{
	int udp_send_msg_len = QUIC_HDR_LEN_HASH + STREAM_FRAME_LEN +
			       INCHOATE_CHLO_LEN + CLIENTHELLO_MIN_SIZE;
	...
	module_quic_chlo.packet_length =
	    sizeof(struct ether_header) + sizeof(struct ip) +
			  ...
			  int probe_num, UNUSED void *arg)
{
	...
	common_hdr->quic_version = MakeQuicTag('Q', '0', '4', '3');
	...
	quic_stream_frame_packet *frame =
	    (quic_stream_frame_packet *)(payload + payload_len);
	...
	quic_inchoate_chlo *chlo =
	    (quic_inchoate_chlo *)(payload + payload_len);
	...
	payload_len += pad_len;
	value_data += pad_len;
	*((uint32_t *)value_data) = MakeQuicTag('Q', '0', '4', '3');
			      ...
			      fieldset_t *fs, UNUSED uint32_t *validation)
{
	...
	if (ip_hdr->ip_p == IPPROTO_UDP) {
		...
		if (data_len > sizeof(struct udphdr)) {
			...
			if (data_len >
			    (QUIC_HDR_LEN_HASH - 13 - sizeof(struct udphdr))) {
				...
				if (data_len <
				    (QUIC_HDR_LEN_HASH + CLIENTHELLO_MIN_SIZE -
...
probe_module_t module_quic_chlo = {
    ...
    .packet_length = sizeof(struct ether_header) + sizeof(struct ip) +
		     sizeof(struct udphdr) + QUIC_HDR_LEN_HASH +
